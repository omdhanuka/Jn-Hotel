...
import { Calendar, Users, Search, Filter, Eye, CheckCircle, XCircle, Clock, DollarSign, BedDouble, Building, AlertTriangle } from 'lucide-react';
...
const StaffBookingManagement: React.FC = () => {
  ...
  const checkPermissions = async () => {
    try {
      ...
      if (!response.data.permissions.viewBookings && !response.data.permissions.manageBookings) {
  ...
  const getTypeIcon = (type: string) => {
    switch (type) {
      case 'room': return <BedDouble className="h-5 w-5" />;
      case 'banquet': return <Building className="h-5 w-5" />;
  ...
  const filteredBookings = bookings.filter(booking => {
    if (filters.search) {
      ...
      if (!guestName.includes(searchLower) && !bookingId.includes(searchLower)) {
        return false;
      }
    }
    return true;
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600 mt-2">
            {permissions.manageBookings ? 'View and manage' : 'View'} hotel bookings
          </p>
        </div>
          ...
          <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-6">
            <div className="flex items-center">
              ...
              <p className="text-sm text-yellow-800">
                You have view-only access. Contact admin to request booking management permissions.
              </p>
            </div>
          </div>
        )}
                    ...
                    <tr key={booking._id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center">
                          ...
                          <span className="ml-2 text-sm capitalize">{booking.type}</span>
                        </div>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm">
                        <div className="flex items-center space-x-2">
                          ...
                          {permissions.manageBookings && booking.status === 'pending' && (
                            <>
                              <button
