...
export const assignComplaint = async (req: AuthRequest, res: Response) => {
  try {
    ...
    // Get booking details if complaint has a booking
    let roomId = null;
    if (complaint.booking) {
      const booking = await Booking.findById(complaint.booking).populate('resourceId');
      if (booking && booking.type === 'room') {
        roomId = booking.resourceId;
      }
    }

    // Create a staff task for the complaint
    const taskType = complaint.category.includes('cleanliness') ? 'cleaning' 
                   : complaint.category.includes('maintenance') ? 'maintenance' 
                   : 'service';
    ...
    const staffTask = new StaffTask({
      ...
      autoGeneratedReason: `Complaint assignment: ${complaint.complaintId}`,
      dueTime: new Date(Date.now() + 24 * 60 * 60 * 1000) // Due in 24 hours
    });
    ...
    complaint.timeline.push({
      ...
      remarks: remarks || `Assigned to ${staff.firstName} ${staff.lastName} - Task created: ${staffTask._id}`
    });
